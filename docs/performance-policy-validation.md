...
identity, version, SHA-256 digest and mode, and exactly one report evaluation for
every policy rule. Policy identity does not imply test identity; the surrounding
registry explicitly binds a policy to its approved test. The approved
baseline-resolution set must exactly cover the distinct baseline IDs and versions
named by regression rules. Metric selectors must match all three policy fields.
